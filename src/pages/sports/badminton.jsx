...
const BadmintonList = () => {
  ...
  useEffect(() => {
    // ব্যাডমিন্টন কালেকশন থেকে ডাটা নেওয়া হচ্ছে
    const q = query(collection(db, "badminton"), orderBy("createdAt", "desc"));
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      const playerData = await Promise.all(snapshot.docs.map(async (playerDoc) => {
        ...
        return {
          id: playerDoc.id,
          ...data, // পজিশন ও অভিজ্ঞতা
          // প্রোফাইল থেকে কমন ডাটা সেট
  ...
  return (
    <div className="min-h-screen  text-slate-900 dark:text-white font-bangla pb-24 transition-all">
      ...
      <div className="sticky top-0 z-40  backdrop-blur-xl border-b dark:border-white/5 px-4 py-4 flex items-center justify-center">
         <h1 className="text-xl font-black italic uppercase tracking-tighter">Badminton <span className="text-blue-600">Players</span></h1>
      </div>
              ...
              <div key={player.id} className="bg-white dark:bg-[#0D0D0D] rounded-[2rem] border border-slate-100 dark:border-white/5 p-5 shadow-sm">
                    ...
                    <div className="mt-4 space-y-2">
                      <div className="flex items-center justify-between bg-slate-50 dark:bg-white/5 p-2 px-3 rounded-xl border border-slate-100 dark:border-white/10">
                        <div className="flex items-center gap-2 text-slate-500">
                          ...
                          <span className="text-[10px] font-bold uppercase">Position</span>
                        </div>
                ...
                <div className="mt-5 flex gap-3">
                  <a 
                    ...
                    className="flex-1 h-12 rounded-2xl bg-blue-600 text-white flex items-center justify-center gap-2 font-black text-sm active:scale-95 transition-all"
                  >
                  ...
                  <a 
                    ...
                    className="flex-1 h-12 rounded-2xl bg-emerald-500 text-white flex items-center justify-center gap-2 font-black text-sm active:scale-95 transition-all"
                  >
      ...
      <button 
        ...
        className="fixed bottom-24 right-6 w-12 h-12 bg-blue-600 text-white rounded-2xl shadow-2xl flex items-center justify-center z-50"
      >
