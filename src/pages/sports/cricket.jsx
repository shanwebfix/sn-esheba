...
const CricketList = () => {
  ...
  useEffect(() => {
    // আপনার DB নাম 'cricket' ই রাখা হয়েছে
    const q = query(collection(db, "cricket"), orderBy("createdAt", "desc"));

    const unsubscribe = onSnapshot(q, async (snapshot) => {
      const playerData = await Promise.all(snapshot.docs.map(async (playerDoc) => {
        const data = playerDoc.data();
        
        // ইউজারের মূল প্রোফাইল (users collection) থেকে কমন ডাটা আনা হচ্ছে
        let profileData = {};
        if (data.userId) {
        ...
        return {
          id: playerDoc.id,
          ...data, // ক্রিকেট ডাটা (position, experience)
          // প্রোফাইল থেকে কমন ডাটা ওভাররাইট করা হচ্ছে
  ...
  return (
    <div className="min-h-screen  text-slate-900 dark:text-white font-bangla pb-24 transition-all">
      ...
      <div className="sticky top-0 z-40  backdrop-blur-xl border-b dark:border-white/5 px-4 py-4 flex items-center justify-center">
         <h1 className="text-xl font-black italic uppercase tracking-tighter">Cricket <span className="text-blue-600">Players</span></h1>
      </div>
      ...
      <div className="max-w-md mx-auto px-0 pt-6">
        ...
        <div className="space-y-4">
          {filteredPlayers.map((player) => (
            <div key={player.id} className="bg-white dark:bg-[#0D0D0D] rounded-[2rem] border border-slate-100 dark:border-white/5 p-5 shadow-sm">
              <div className="flex gap-4">
                <img 
                  src={player.photoURL || `https://ui-avatars.com/api/?name=${player.name}`} 
                  className="w-20 h-20 rounded-2xl object-cover border-2 border-blue-600/10"
                  alt="Profile"
                />
                <div className="flex-grow">
                  <h3 className="font-black text-lg">{player.name}</h3>
                  <div className="flex flex-wrap gap-2 mt-1">
                    <span className="text-[10px] font-black bg-blue-50 dark:bg-blue-900/30 text-blue-600 px-2 py-0.5 rounded-full">
                    ...
                    <span className="text-[10px] font-black bg-slate-100 dark:bg-gray-800 text-slate-500 px-2 py-0.5 rounded-full flex items-center gap-1">
                      <MapPin size={8} /> {player.village}
                    </span>
                  </div>

                  <div className="mt-4 space-y-2">
                    <div className="flex items-center justify-between bg-slate-50 dark:bg-white/5 p-2 px-3 rounded-xl">
                      <span className="text-[10px] font-bold text-slate-500 uppercase">Position</span>
                      <span className="text-xs font-black text-blue-600">{player.position}</span>
                    </div>
                    <div className="flex items-center justify-between bg-slate-50 dark:bg-white/5 p-2 px-3 rounded-xl">
              ...
              <div className="mt-5 flex gap-3">
                <a href={`tel:${player.phone}`} className="flex-1 h-12 rounded-xl bg-blue-600 text-white flex items-center justify-center gap-2 font-black text-sm">
                  <Phone size={16} fill="currentColor" /> কল করুন
                </a>
                <a href={`https://wa.me/88${player.phone}`} target="_blank" rel="noopener noreferrer" className="flex-1 h-12 rounded-xl bg-emerald-500 text-white flex items-center justify-center gap-2 font-black text-sm">
