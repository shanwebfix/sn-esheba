...
const MuajjinList = () => {
  ...
  useEffect(() => {
    // মুয়াজ্জিন কালেকশন থেকে ডাটা আনা হচ্ছে
    const q = query(collection(db, "muajjin"));
    
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      try {
        const baseData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));

        // প্রোফাইল (users) টেবিল থেকে কমন ডাটা সিঙ্ক করা
    ...
    <div className="min-h-screen font-bangla pb-24 text-slate-900 dark:text-white ">
      ...
      <div className=" backdrop-blur-md px-6 py-5 border-b dark:border-white/5 sticky top-0 z-10 text-center">
        <h1 className="text-xl font-black ">
          শমশেরনগর এর সকল <span className="text-blue-600">মুয়াজ্জিন</span> এর লিস্ট
        </h1>
      </div>
      ...
      <div className="max-w-md mx-auto px-0 mt-6">
        ...
        <div className="space-y-5">
          {filteredData.map((item) => (
            <div key={item.id} className="bg-white dark:bg-zinc-900 rounded-[2.5rem] p-5 border dark:border-white/5 shadow-sm hover:shadow-md transition-all">
              <div className="flex gap-4">
                <div className="relative">
                  <img 
                    src={item.photoURL || `https://ui-avatars.com/api/?name=${item.name}&background=0369a1&color=fff`} 
                    className="w-20 h-20 rounded-3xl object-cover border-2 border-slate-100 dark:border-white/10" 
                    alt={item.name} 
                  />
                  <div className="absolute -bottom-1 -right-1 bg-blue-500 text-white p-1 rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  <span className="text-[9px] bg-blue-100 dark:bg-blue-900/30 text-blue-600 px-2.5 py-1 rounded-lg font-black uppercase tracking-widest inline-block mt-1.5">
                    {item.category || "Muajjin"}
                  </span>

                  {/* মসজিদ */}
                  <div className="mt-3 flex items-start gap-1.5 text-emerald-600 dark:text-emerald-400 text-xs font-black leading-tight">
                    <Lucide.Home size={14} className="mt-0.5 shrink-0" /> 
                  ...
                  <div className="flex items-center gap-1.5 text-slate-400 text-[10px] font-bold mt-2 uppercase tracking-tight">
                    <Lucide.MapPin size={12} /> {item.village || "ঠিকানা নেই"}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 mt-5 pt-4 border-t dark:border-white/5">
                <a 
                  href={`tel:${item.phone}`} 
                  className="flex items-center justify-center gap-2 py-3.5 bg-slate-900 dark:bg-white text-white dark:text-black rounded-2xl text-[12px] font-black active:scale-95 transition-all"
                >
                ...
                <a 
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center justify-center gap-2 py-3.5 bg-[#25D366] text-white rounded-2xl text-[12px] font-black active:scale-95 transition-all"
                >
          ...
          {filteredData.length === 0 && (
            <div className="text-center py-20 opacity-30">
              ...
              <p className="font-bold mt-2 uppercase tracking-widest text-xs">কোন ডাটা পাওয়া যায়নি</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="fixed bottom-24 right-6 z-50">
        <button 
          ...
          className="w-14 h-14 bg-blue-600 text-white rounded-2xl shadow-2xl shadow-blue-500/40 flex items-center justify-center hover:scale-110 active:scale-90 transition-all"
        >
