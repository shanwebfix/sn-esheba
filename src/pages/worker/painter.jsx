...
import { 
  ...
  PhoneCall, Plus, Loader2, Palette, 
  Paintbrush, ShieldCheck, Info, Sparkles 
} from 'lucide-react';

const PainterDirectory = () => {
  ...
  // --- ডাটা লজিক: প্রোফাইল থেকে কমন ডাটা আনা ---
  useEffect(() => {
    const q = query(collection(db, "painters"), orderBy("createdAt", "desc"));
        ...
        const enrichedWorkers = await Promise.all(baseData.map(async (worker) => {
          if (worker.userId) {
            ...
            if (userDoc.exists()) {
              ...
              return {
                ...worker,
                // প্রোফাইল থেকে লাইভ ডাটা ওভাররাইড করা হচ্ছে
    ...
    <div className="min-h-screen py-10 px-4 font-bangla transition-colors duration-300 pb-24">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center p-4 bg-indigo-600 rounded-3xl text-white mb-4 shadow-xl shadow-indigo-200 dark:shadow-none">
            <Palette size={35} />
          </div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white mb-2">পেইন্টার ও রং মিস্ত্রি</h1>
          <p className="text-slate-500 font-medium">দক্ষ কারিগর দিয়ে আপনার বাড়িকে দিন নতুন রূপ</p>
        </div>
        ...
        <div className="max-w-md mx-auto mb-12 relative">
          <input 
            ...
            className="w-full py-4 pl-12 pr-4 bg-white dark:bg-gray-900 border border-indigo-100 dark:border-gray-800 rounded-2xl shadow-sm outline-none focus:ring-4 focus:ring-indigo-500/10 font-bold dark:text-white transition-all"
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredWorkers.map((worker) => (
              <div key={worker.id} className="bg-white dark:bg-gray-900 rounded-[2.5rem] p-6 border border-indigo-50 dark:border-gray-800 shadow-sm hover:shadow-xl transition-all group relative overflow-hidden">
                
                {/* Profile Section */}
                <div className="flex items-center gap-5 mb-4 cursor-pointer relative z-10" onClick={() => setSelectedWorker(worker)}>
                  <div className="relative">
                    <img 
                      src={worker.photoURL || `https://ui-avatars.com/api/?name=${worker.name}&background=4f46e5&color=fff`} 
                      className="w-20 h-20 rounded-2xl object-cover border-2 border-indigo-100 dark:border-gray-700 shadow-md" 
                      alt="" 
                    />
                  ...
                  <div>
                    ...
                    <span className="text-indigo-600 dark:text-indigo-400 font-bold text-[10px] uppercase bg-indigo-50 dark:bg-indigo-900/30 px-2 py-0.5 rounded-md mt-1 inline-block">
                      {worker.category || "প্রফেশনাল পেইন্টার"}
                    </span>
                  </div>
                </div>

                <p className="text-xs text-slate-500 dark:text-gray-400 mb-4 line-clamp-2 font-medium px-1 relative z-10">
                  {worker.specialty || "ইন্টেরিয়র ও এক্সটেরিয়র রং, প্লাস্টিক পেইন্ট এবং বার্নিশ কাজের দক্ষ মিস্ত্রি।"}
                </p>

                <div className="space-y-2 mb-6 text-sm text-slate-600 dark:text-gray-400 font-medium relative z-10">
                  <div className="flex items-center gap-2 px-1"><MapPin size={16} className="text-red-500" /> {worker.location}</div>
                  <div className="flex items-center gap-2 px-1"><Sparkles size={16} className="text-indigo-500" /> {worker.experience} অভিজ্ঞতা</div>
                </div>
        ...
        {!loading && filteredWorkers.length === 0 && (
          <div className="text-center py-20 text-slate-400 font-bold">কোন রং মিস্ত্রি পাওয়া যায়নি!</div>
        )}
      </div>
          ...
          <div className="bg-white dark:bg-gray-900 w-full max-w-md rounded-[3rem] overflow-hidden relative animate-in zoom-in-95 shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-8 pt-12 overflow-y-auto flex-grow custom-scrollbar">
              <div className="text-center mb-8">
                <img src={selectedWorker.photoURL || `https://ui-avatars.com/api/?name=${selectedWorker.name}&background=4f46e5&color=fff`} className="w-28 h-28 rounded-[2.5rem] object-cover border-4 border-indigo-50 dark:border-gray-800 mx-auto mb-4 shadow-xl" alt="" />
                <h2 className="text-2xl font-black dark:text-white leading-tight">{selectedWorker.name}</h2>
                <p className="text-indigo-600 font-black text-sm uppercase mt-2">{selectedWorker.category || "মাস্টার পেইন্টার"}</p>
              </div>

              <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-3xl border border-indigo-100 dark:border-indigo-900/40 text-center">
                  <Paintbrush className="mx-auto mb-1.5 text-indigo-600" size={24} />
                ...
                <div className="bg-emerald-50 dark:bg-emerald-900/20 p-4 rounded-3xl border border-emerald-100 dark:border-emerald-900/40 text-center">
                  ...
                  <p className="font-black dark:text-white text-[10px] leading-tight mt-1">প্রিমিয়াম ফিনিশিং</p>
                </div>
              </div>
              ...
              <div className="bg-slate-50 dark:bg-gray-800/50 p-6 rounded-[2rem] mb-8 border border-slate-100 dark:border-gray-800">
                ...
                <p className="font-bold dark:text-gray-200 text-xs leading-relaxed">
                  {selectedWorker.specialty || "কোনো বিস্তারিত তথ্য দেওয়া নেই।"}
                </p>
              </div>
            </div>
          </div>
...
export default PainterDirectory;