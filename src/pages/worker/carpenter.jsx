...
import { 
  ...
  PhoneCall, Plus, Loader2, Hammer, 
  Axe, ShieldCheck, Info, Sofa
} from 'lucide-react';

const CarpenterDirectory = () => {
  ...
  // --- ডাটা ফেচিং: প্রোফাইল (Common) + কার্পেন্টার (Specific) ---
  useEffect(() => {
    // এখানে কালেকশন নাম 'carpenters' ব্যবহার করা হয়েছে
    const q = query(collection(db, "carpenters"), orderBy("createdAt", "desc"));
        ...
        const enrichedWorkers = await Promise.all(baseData.map(async (worker) => {
          if (worker.userId) {
            ...
            if (userDoc.exists()) {
              ...
              return {
                ...worker,
                // প্রোফাইল থেকে কমন ডাটা ওভাররাইড করা হচ্ছে
    ...
    <div className="min-h-screen py-10 px-4 font-bangla transition-colors duration-300 pb-24">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center p-4 bg-amber-800 rounded-3xl text-white mb-4 shadow-xl shadow-amber-200 dark:shadow-none">
            <Hammer size={35} />
          </div>
          <h1 className="text-3xl font-black text-amber-950 dark:text-white mb-2">কার্পেন্টার ও কাঠ মিস্ত্রি</h1>
          <p className="text-amber-700/70 dark:text-gray-400 font-medium">আধুনিক ফার্নিচার এবং কাঠের কারুকাজের নির্ভরযোগ্য কারিগর</p>
        </div>
        ...
        <div className="max-w-md mx-auto mb-12 relative">
          <input 
            ...
            className="w-full py-4 pl-12 pr-4 bg-white dark:bg-gray-900 border border-amber-100 dark:border-gray-800 rounded-2xl shadow-sm outline-none focus:ring-4 focus:ring-amber-500/10 font-bold dark:text-white transition-all"
            onChange={(e) => setSearchQuery(e.target.value)}
          />
          <Search className="absolute left-4 top-4 text-amber-400" size={20} />
        </div>

        {/* Workers Grid */}
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            <Loader2 className="animate-spin text-amber-800 mb-4" size={40} />
            <p className="text-amber-800 font-bold">লোড হচ্ছে...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredWorkers.map((worker) => (
              <div key={worker.id} className="bg-white dark:bg-gray-900 rounded-[2.5rem] p-6 border border-amber-100 dark:border-gray-800 shadow-sm hover:shadow-xl transition-all group relative">
                
                {/* Profile Section */}
                <div className="flex items-center gap-5 mb-4 cursor-pointer" onClick={() => setSelectedWorker(worker)}>
                  <div className="relative">
                    <img 
                      src={worker.photoURL || `https://ui-avatars.com/api/?name=${worker.name}&background=92400e&color=fff`} 
                      className="w-20 h-20 rounded-2xl object-cover border-2 border-amber-100 dark:border-gray-700 shadow-md" 
                      alt="" 
                    />
                    <div className="absolute -bottom-2 -right-2 bg-amber-800 text-white p-1.5 rounded-xl shadow-lg">
                      <Axe size={14} />
                    </div>
                  </div>
                  <div>
                    <h3 className="text-xl font-black dark:text-white leading-tight">{worker.name}</h3>
                    <span className="text-amber-800 dark:text-amber-500 font-bold text-[10px] uppercase bg-amber-50 dark:bg-amber-900/30 px-2 py-0.5 rounded-md mt-1 inline-block">
                      {worker.category || "ফার্নিচার মেকার"}
                    </span>
                  </div>
                </div>

                <p className="text-xs text-slate-500 dark:text-gray-400 mb-4 line-clamp-2 font-medium px-1">
                  {worker.specialty || "খাট, আলমারি মেরামত ও নতুন তৈরির কাজ এবং কাঠের দরজা-জানালার কাজ করা হয়।"}
                </p>
                ...
                <div className="space-y-2 mb-6 text-sm text-slate-600 dark:text-gray-400 font-medium">
                  ...
                  <div className="flex items-center gap-2 px-1"><Hammer size={16} className="text-amber-600" /> {worker.experience} অভিজ্ঞতা</div>
                </div>
        ...
        {!loading && filteredWorkers.length === 0 && (
          <div className="text-center py-20 text-slate-400 font-bold">কোন কাঠ মিস্ত্রি পাওয়া যায়নি!</div>
        )}
      </div>
      ...
      <button 
        ...
        className="fixed bottom-24 right-8 w-12 h-12 bg-amber-800 text-white rounded-2xl shadow-2xl flex items-center justify-center hover:scale-110 active:scale-90 transition-all z-50 "
      >
          ...
          <div className="bg-white dark:bg-gray-900 w-full max-w-md rounded-[3rem] overflow-hidden relative animate-in zoom-in-95 shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-8 pt-12 overflow-y-auto flex-grow custom-scrollbar">
              <div className="text-center mb-8">
                <img src={selectedWorker.photoURL || `https://ui-avatars.com/api/?name=${selectedWorker.name}&background=92400e&color=fff`} className="w-28 h-28 rounded-[2.5rem] object-cover border-4 border-amber-50 dark:border-gray-800 mx-auto mb-4 shadow-xl" alt="" />
                <h2 className="text-2xl font-black dark:text-white leading-tight">{selectedWorker.name}</h2>
                <p className="text-amber-800 font-black text-sm uppercase mt-2">{selectedWorker.category || "কাঠের কারিগর"}</p>
              </div>

              <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-3xl border border-amber-100 dark:border-amber-900/40 text-center">
                  <Sofa className="mx-auto mb-1.5 text-amber-800" size={24} />
...
export default CarpenterDirectory;