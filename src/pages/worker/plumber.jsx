...
import { 
  ...
  MapPin, PhoneCall, Plus, Loader2, Droplets, 
  Waves, ShieldCheck, Info
} from 'lucide-react';

const PlumberDirectory = () => {
  ...
  // --- ফায়ারবেস লজিক আপডেট ---
  useEffect(() => {
    const q = query(collection(db, "plumbers"), orderBy("createdAt", "desc"));
    
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      try {
        const baseData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));

        // প্রতিটি মিস্ত্রির userId দিয়ে প্রোফাইল থেকে কমন ডাটা আনা
    ...
    <div className="min-h-screen py-10 px-4 font-bangla transition-colors duration-300 pb-24">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center p-4 bg-cyan-600 rounded-3xl text-white mb-4 shadow-xl">
            <Droplets size={35} />
          </div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white mb-2">প্লাম্বার ও স্যানিটারি মিস্ত্রি</h1>
          <p className="text-slate-500 font-medium">পানির লাইন ও স্যানিটারি সমস্যার দ্রুত সমাধান</p>
        </div>
        ...
        <div className="max-w-md mx-auto mb-12 relative">
          <input 
            ...
            className="w-full py-4 pl-12 pr-4 bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-2xl shadow-sm outline-none focus:ring-4 focus:ring-cyan-500/10 font-bold dark:text-white transition-all"
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredWorkers.map((worker) => (
              <div key={worker.id} className="bg-white dark:bg-gray-900 rounded-[2.5rem] p-6 border border-slate-100 dark:border-gray-800 shadow-sm hover:shadow-xl transition-all group relative">
                
                {/* Profile Section */}
                <div className="flex items-center gap-5 mb-4 cursor-pointer" onClick={() => setSelectedWorker(worker)}>
                  <div className="relative">
                    <img 
                      src={worker.photoURL || `https://ui-avatars.com/api/?name=${worker.name}&background=06b6d4&color=fff`} 
                      className="w-20 h-20 rounded-2xl object-cover border-2 border-cyan-100 dark:border-gray-700 shadow-md" 
                      alt="" 
                    />
                    <div className="absolute -bottom-2 -right-2 bg-cyan-600 text-white p-1.5 rounded-xl shadow-lg">
                      <Waves size={14} />
                    </div>
                  </div>
                  <div>
                    <h3 className="text-xl font-black dark:text-white leading-tight">{worker.name}</h3>
                    <span className="text-cyan-600 font-bold text-[10px] uppercase bg-cyan-50 dark:bg-cyan-900/30 px-2 py-0.5 rounded-md mt-1 inline-block">
                      {worker.category || "প্লাম্বিং মিস্ত্রি"}
                    </span>
                  </div>
                </div>

                <p className="text-xs text-slate-500 dark:text-gray-400 mb-4 line-clamp-2 font-medium px-1">
                  {worker.specialty || "মোটর মেরামত, পাইপ ফিটিং এবং বাথরুম স্যানিটারি কাজের অভিজ্ঞ মিস্ত্রি।"}
                </p>
                ...
                <div className="space-y-2 mb-6 text-sm text-slate-600 dark:text-gray-400 font-medium">
                  ...
                  <div className="flex items-center gap-2 px-1"><Wrench size={16} className="text-cyan-500" /> {worker.experience} অভিজ্ঞতা</div>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <a 
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex-1 py-3.5 bg-[#25D366] text-white rounded-xl font-black text-[12px] transition-all active:scale-95 flex items-center justify-center gap-2 shadow-lg shadow-green-500/10"
                  >
        ...
        {!loading && filteredWorkers.length === 0 && (
          <div className="text-center py-20 text-slate-400 font-bold">কোন প্লাম্বার পাওয়া যায়নি!</div>
        )}
      </div>
      ...
      <button 
        ...
        className="fixed bottom-24 right-8 w-12 h-12 bg-cyan-600 text-white rounded-2xl shadow-2xl shadow-cyan-500/40 flex items-center justify-center hover:scale-110 active:scale-90 transition-all z-50 "
      >
          ...
          <div className="bg-white dark:bg-gray-900 w-full max-w-md rounded-[3rem] overflow-hidden relative animate-in zoom-in-95 shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-8 pt-12 overflow-y-auto flex-grow custom-scrollbar">
              <div className="text-center mb-8">
                <img src={selectedWorker.photoURL || `https://ui-avatars.com/api/?name=${selectedWorker.name}&background=06b6d4&color=fff`} className="w-28 h-28 rounded-[2.5rem] object-cover border-4 border-cyan-50 dark:border-gray-800 mx-auto mb-4 shadow-xl" alt="" />
                <h2 className="text-2xl font-black dark:text-white leading-tight">{selectedWorker.name}</h2>
                <p className="text-cyan-600 font-black text-sm uppercase mt-2">{selectedWorker.category || "প্লাম্বিং এক্সপার্ট"}</p>
              </div>

              <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="bg-cyan-50 dark:bg-cyan-900/20 p-4 rounded-3xl border border-cyan-100 dark:border-cyan-900/40 text-center">
                  <Wrench className="mx-auto mb-1.5 text-cyan-600" size={24} />
                ...
                <div className="bg-emerald-50 dark:bg-emerald-900/20 p-4 rounded-3xl border border-emerald-100 dark:border-emerald-900/40 text-center">
                  ...
                  <p className="text-[10px] text-slate-400 font-black uppercase">ভেরিফাইড</p>
                  <p className="font-black dark:text-white text-[10px] leading-tight mt-1">সুরক্ষিত সেবা</p>
                </div>
              </div>
...
export default PlumberDirectory;